...
def on_message(topic, payload):
    print("Topic : ", topic)
    print("message received", str(payload.decode("utf-8")))


# 메시지 전송
def msg_send(mqttc, topic, msg):
    mqttc.publish(topic, msg, qos=mqtt.QoS.AT_LEAST_ONCE)
...
def main():
    ...
    ##mqtt연결
    mqttc = build_mqtt_connection()
    mqttc.connect().result()
    mqttc.subscribe("status/#", qos=mqtt.QoS.AT_LEAST_ONCE, callback=on_message).result()
